...
import StructuralSearchComponent from '../searchpage/structural';
...
const Structural = () => {
  ...
  const handleSave = async () => {
    if (butMode === 'create') {
      ...
      const objReq = {
        ...
        body: `drawingTitle=${drawingTitle}&drawingCode=${drawingCode}&drawingLocation=${drawingLocation}&trade=${trade}&uploadFiles=${uploadFiles}`,
      };

      const data = await apiRequest('http://localhost:5000/save-structural', objReq);
      console.log(data);
      ...
      const data = await apiRequest(`http://localhost:5000/update-structural/${itemId}`, objReq);
      console.log(data);
  ...
  const handleReadData = async () => {
    const response = await fetch('http://localhost:5000/get-structural-data');
  ...
  const deleteDrawing = async (id) => {
    ...
    if (window.confirm(text) === true) {
      ...
      await apiRequest(`http://localhost:5000/delete-structural/${id}`, objReq);
      handleReadData();
    }
  };
  ...
  const updateItem = async (id) => {
    ...
    setButname('Update');
    setSetItemId(id);
    const response = await fetch(`http://localhost:5000/get-structural/${id}`);
    const data = await response.json();
