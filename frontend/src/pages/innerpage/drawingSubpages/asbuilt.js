...
import AsbuiltSearchComponent from '../searchpage/asbuilt';
...
const Asbuilt = () => {
  ...
  const handleSave = async () => {
    if (butMode === 'create') {
      ...
      const data = await apiRequest('http://localhost:5000/save-asbuilt', objReq);
      console.log(data);
      ...
      const data = await apiRequest(`http://localhost:5000/update-asbuilt/${itemId}`, objReq);
      console.log(data);
  ...
  const handleReadData = async () => {
    const response = await fetch('http://localhost:5000/get-asbuilt-data');
  ...
  const deleteDrawing = async (id) => {
    ...
    if (window.confirm(text) === true) {
      ...
      await apiRequest(`http://localhost:5000/delete-asbuilt/${id}`, objReq);
      handleReadData();
    }
  };
  ...
  const updateItem = async (id) => {
    ...
    setButname('Update');
    setSetItemId(id);
    const response = await fetch(`http://localhost:5000/get-asbuilt/${id}`);
    const data = await response.json();
