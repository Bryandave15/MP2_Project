...
import ArchiSearchComponent from '../searchpage/architectural';
...
const Architectural = () => {
  ...
  const handleSave = async () => {
    if (butMode === 'create') {
      ...
      const data = await apiRequest('http://localhost:5000/archi', objReq);
      console.log(data);
      ...
      const data = await apiRequest(`http://localhost:5000/update-archi/${itemId}`, objReq);
      console.log(data);
  ...
  const handleReadData = async () => {
    const response = await fetch('http://localhost:5000/get-archi-data');
  ...
  const deleteDrawing = async (id) => {
    ...
    if (window.confirm(text) === true) {
      ...
      await apiRequest(`http://localhost:5000/delete-archi/${id}`, objReq);
      handleReadData();
    }
  };
  ...
  const updateItem = async (id) => {
    ...
    setButname('Update');
    setSetItemId(id);
    const response = await fetch(`http://localhost:5000/get-archi/${id}`);
    const data = await response.json();
